...
- Inside `server/`: `npm run typecheck` (just `tsc --noEmit`). There is no unit-test runner in `server/`, but five self-check scripts act as the backend quality gates: `npm run ranker:selfcheck` (learned-ranker sanity over `scripts/ranker-selfcheck.ts`), `npm run agents:selfcheck` (TS agentic layer), `npm run twin:selfcheck` (digital-twin collectors/predictor), `npm run memory:selfcheck` (memory-layer regression gate; opens a throwaway DB via `BRAIN_DB_PATH`), and `npm run perception:selfcheck` (Phase 3 — `0002-cognitive-abstractions-level` migration + abstraction classifier + worker-down graceful degrade). Run the relevant one after touching `reasoning/ranker*`, `agents/`, `twin/`, `memory/`, or `perception/`/`core/abstractionLevels.ts`.
