...
| H3 Imagination→decision | ⏸ deferred | — | The ReAct agent loop emits ONE model-chosen tool per round, with no natural candidate set to re-rank. Needs a design choice — (a) an *advisory foresight* step that simulates the chosen action's predicted outcome (via `causalMap`/`imagination`) and feeds a warning back into the loop, or (b) changing the loop's protocol to emit N candidate intentions for imagination to rank. Deferred rather than forced into a poor fit. |
