...
| H3 Imagination→decision | ✅ done as variant (a), gate-green | `agent:selfcheck` (foresight section) | *Advisory foresight*: before the loop executes a proposed tool, the empirically-learned causal map (`getEffectsForCause("action:<id>")`) is formatted into ONE bounded `Foresight:` transcript line (`consequences.foresightLine`, pure) the model sees next round — once per action per run, fail-open, zero LLM/extra-DB cost, gated by the maturation "imagination" feature (stage 4 / `ENABLE_PER_REQUEST_IMAGINATION`). Variant (b) — N candidate intentions ranked by imagination — remains roadmap; it needs a loop-protocol change. |

MEDIUM/LOW/FUTURE remain roadmap. The full `npm run gate` (75 steps) is green with all six slices wired in.
